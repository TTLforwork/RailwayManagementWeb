-- 1. Drop the database if it exists and create a fresh one
...
-- Set engine for InnoDB to support foreign keys and transactions
SET default_storage_engine = InnoDB;

-- Drop tables in reverse dependency order (Idempotency)
-- NOTE: MySQL uses IF EXISTS for clean drops
...
-- FIX: MySQL reserves 'User', so backticks must be used: `User`
...
-- Create Passenger table (PNR widened for resilience.)
...
CREATE TABLE Ticket (
    ...
    -- FIX: Changed GETDATE() to CURRENT_TIMESTAMP
...
CREATE TABLE Feedback (
    -- FIX: Changed IDENTITY(1,1) to AUTO_INCREMENT
...
('john_doe', 'T001', 5, 'Excellent service! Very comfortable journey.', 'Thank thank you for your positive feedback!', '2025-11-21 10:00:00'),
